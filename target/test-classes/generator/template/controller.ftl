...
public class ${modelNameUpperCamel}Controller {
    ...
    @PostMapping("/list")
    public Result list(@RequestParam(defaultValue = ProjectConstant.PAGE_INDEX) Integer pageIndex, @RequestParam(defaultValue = ProjectConstant.PAGE_SIZE) Integer pageSize,@RequestParam(required = false,defaultValue = "") String searchCondition) {
        PageHelper.startPage(pageIndex, pageSize);
        Condition condition = new Condition(${modelNameUpperCamel}.class);
        List<${modelNameUpperCamel}> list = ${modelNameLowerCamel}Service.findByCondition(ConditionUtils.getConditions(condition,searchCondition));
